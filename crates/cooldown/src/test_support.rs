...
///
/// Every project root reaches coordination, discovery, and recovery through
/// `std::fs::canonicalize`, so the paths those entry points accept and echo back are canonical.
...
pub(crate) fn canonical_path(path: &Utf8Path) -> eyre::Result<Utf8PathBuf> {
    Utf8PathBuf::from_path_buf(std::fs::canonicalize(path)?)
        .map_err(|path| eyre::eyre!("canonical path is not UTF-8: {}", path.display()))
}

/// Returns `directory`'s canonical root, standing in for the canonical workdir the CLI resolves.
///
/// Fixture trees build from the returned root rather than the raw handout, which keeps the test
/// on the same footing as production: `cli::setup::detect::workdir` canonicalizes before anything
/// derives a scope from it, and scope membership is a prefix test that only holds between two
/// canonical paths.
