//! Swift package version semantics via the `semver` crate. `SwiftPM` requires `SemVer` tags, so the
//! model matches npm/cargo: this module supplies the ordering, the update kind, and the
...
///
/// ```
/// use cooldown_swift::version::parse;
...
///
/// ```
/// use cooldown_swift::version::is_prerelease;
...
///
/// ```
/// use cooldown_swift::version::major_key;
/// use cooldown_core::MajorKey;
/// assert_eq!(major_key("1.2.3"), MajorKey("1".into()));
/// assert_eq!(major_key("0.5.0"), MajorKey("0.5".into()));
/// ```
#[must_use]
...
///
/// ```
/// use cooldown_swift::version::classify_kind;
...
///
/// ```
/// use cooldown_swift::version::compare;
/// use std::cmp::Ordering;
...
/// Strips a leading `v` from a release tag (`v1.2.0` → `1.2.0`), which many Swift packages use.
#[must_use]
...
mod tests {
    ...
    fn semver_and_tags() {
        ...
        assert_eq!(strip_tag_prefix("version"), "version"); // not a version tag
    }
}
